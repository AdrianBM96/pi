...
import { type NotifyOutput, notifyAction } from "./protocol.ts";

export interface ClientActions {
	invoke<TInputSchema extends TSchema, TOutputSchema extends TSchema>(
		action: {
			id: string;
			version: number;
			input: TInputSchema;
...
async function notifyClient(
	...
	return context.clientActions.invoke(notifyAction, { message: input.message.trim() }, { signal });
}
